...
public final class TeleportPlayerScreen extends BaseScreen {
    ...
    public void drawBackground(GuiGraphics graphics, Theme theme, int x, int y, int width, int height) {
        theme.drawGui(graphics, x, y, width, height, WidgetType.NORMAL);
        theme.drawString(graphics, Component.translatable("screen.paper_plane.choose_player"), x + 10, y + 9);
        if (players.isEmpty()) {
            theme.drawString(graphics, Component.translatable("screen.paper_plane.no_players"), x + 10, y + 32);
        }
    }
