...
public record OpenTeleportScreenPacket(UUID sessionId, List<PlayerEntry> players, boolean enderPlane) {
    ...
    public static OpenTeleportScreenPacket decode(FriendlyByteBuf buffer) {
        ...
        if (size < 0 || size > MAX_PLAYERS) {
            throw new IllegalArgumentException("Invalid Paper Plane player list size: " + size);
        }
    ...
    public void encode(FriendlyByteBuf buffer) {
        if (players.size() > MAX_PLAYERS) {
            throw new IllegalArgumentException("Paper Plane player list exceeds " + MAX_PLAYERS);
        }
        buffer.writeUUID(sessionId);
        buffer.writeVarInt(players.size());
        players.forEach(player -> player.encode(buffer));
        buffer.writeBoolean(enderPlane);
    }
}
